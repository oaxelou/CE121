 ...
 * May 19th, 2016
 *   
 * Communication between two users. (semaphores)
 */
...
int main(int argc,char *argv[]){
	...
	if(shmid == -1){
		if(errno != EEXIST){
			...
			semid = my_semget(key, 0);
		}
	}else{
		user_id = FIRST_USR;
		
		//dimiourgia msgqueue
		semid = my_semget(key, IPC_CREAT | IPC_EXCL | S_IRWXU);
		init_sem(semid);
	}
	...
	if(user_id == FIRST_USR){ 
		...
		*point_segm = '1';
		...
		my_semop(semid, SEM_READY, -1); //mplokarei mexri na etoimastei o deuteros xristis
	}
	...
	while(1){
		...
		my_semop(semid, write_send, 1); //ksemplokarei tin read diergasia tou allou xristi
		
		if(!strcmp(buffer, "quit")){
			break;
		}
		
		my_semop(semid, write_rcv, -1); //mplokarei mexri na diavasei o allos to minuma pou esteile
	}
